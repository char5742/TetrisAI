...
function predict(model,
    (board, minopos, ren, btb, tspin)::Tuple{Array{T,4},Array{T,4},Array{T,2},Array{T,2},Array{T,2}}
) where {T}
    if (isa(model.ps[1][1].weight, CUDA.CuArray))
        model.model((board, minopos, ren, btb, tspin) |> gpu, model.ps, model.st) |> cpu
    else
        model.model((board, minopos, ren, btb, tspin), model.ps, model.st) |> cpu
    end
end


function predict(model,
    (board, minopos, ren, btb, tspin, holdnext)::Tuple{Array{T,4},Array{T,4},Array{T,2},Array{T,2},Array{T,2},Array{T,3}}
) where {T}
    if (isa(model.ps[1][1].weight, CUDA.CuArray))
        data = (board, minopos, ren, btb, tspin, holdnext)
        response = batch_predict(model, data, 16)
        data = nothing
        return response
    else
        response, _ = model.model((board, minopos, ren, btb, tspin, holdnext), model.ps, model.st) |> cpu
        response
    end
end
...
function batch_predict(model, data, batch_size)
    ...
    board_input_prev, minopos, ren_input, back_to_back, tspin, mino_list = data
    ...
    for i in 1:num_batches
        ...
        ren_input_batch = ren_input[:, start_idx:end_idx] 
        ...
        data_batch = (board_input_prev_batch, minopos_batch, ren_input_batch, back_to_back_batch, tspin_batch, mino_list_batch)  |> gpu
