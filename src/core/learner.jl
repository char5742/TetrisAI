...
function calc_expected_rewards(
    brain::Brain,
    multistep_node_list::Vector{Node},
    multistep_next_node_list::Vector{Vector{Node}},
    multistep_reward_list::Vector{Float64},
    multisteps_list::Vector{Int64},
    current_expect_reward_array::Array{Float32,2},
    discount_rate::Float64,
)::Vector{Tuple{Float64,Float64}}
    ϵ = 1e-6
    calc_size = length(multistep_node_list)

    # 次の盤面の最大の価値を算出する
    max_node_list =
        select_node_list(multistep_next_node_list, [node.game_state for node in multistep_node_list], (x...) -> predict(brain.main_model, x))
    # ゲームオーバーではないノードがあるとき、次の盤面の最大の価値を算出する
    if (any(!node.game_state.game_over_flag for node in multistep_node_list))
        node_currentboard_array = [node.game_state.current_game_board.binary .|> Float32 for node in multistep_node_list if !node.game_state.game_over_flag] |> vector2array
        max_node_minopos_array = [generate_minopos(n.mino, n.position) .|> Float32 for n in max_node_list if !isnothing(n)] |> vector2array
        node_combo_array = [node.game_state.combo |> Float32 for node in multistep_node_list if !node.game_state.game_over_flag] |> vector2array
        node_back_to_back_array = [node.game_state.back_to_back_flag |> Float32 for node in multistep_node_list if !node.game_state.game_over_flag] |> vector2array
        max_node_tspin_array = [node.tspin |> Float32 for node in max_node_list if !isnothing(node)] |> vector2array
        node_holdnext = reduce((x, y) -> cat(x, y, dims=3), reshape(hcat([mino_to_array(mino) for mino in [node.game_state.hold_mino, node.game_state.mino_list[end-4:end]...]]...), 7, 6, 1) for node in multistep_node_list if !node.game_state.game_over_flag)
        next_score_list =
    ...
    for i in 1:calc_size
        ...
        node = multistep_node_list[i]
        n = multisteps_list[i]
        reward = multistep_reward_list[i]
        if node.game_state.game_over_flag
            response[i] = (rescaling_reward(reward-1000*discount_rate^n), abs(rescaling_reward(reward-1000*discount_rate^n - inverse_rescaling_reward(current_expect_reward))) + ϵ)
            continue
        end

        p = next_score_list[index]
        index += 1
        temporal_difference = rescaling_reward(reward + inverse_rescaling_reward(p) * discount_rate^n - inverse_rescaling_reward(current_expect_reward))
        expected_reward = rescaling_reward(inverse_rescaling_reward(p) * discount_rate^n + reward)
...
function qlearn(learner::Learner, batch_size, id_and_exp::Vector{Tuple{Int,Experience}}, discount_rate; use_gpu=true)
    try
        ...
        # 次の盤面の価値を予測する為に必要な情報
        multistep_next_node_list_list = Vector{Vector{Node}}(undef, batch_size)
        multistep_node_list = Vector{Node}(undef, batch_size)
        multistep_reward_list = Vector{Float64}(undef, batch_size)
        multisteps_list = Vector{Int64}(undef, batch_size)

        for (i, (id, (;
            current_state,
            selected_node,
            multistep_node,
            multistep_next_node_list,
            multistep_reward,
            multisteps,
            temporal_difference
            ...
            multistep_next_node_list_list[i] = multistep_next_node_list
            multistep_node_list[i] = multistep_node
            multistep_reward_list[i] = multistep_reward
            multisteps_list[i] = multisteps
        end
        ...
        res = calc_expected_rewards(
            learner.brain,
            multistep_node_list,
            multistep_next_node_list_list,
            multistep_reward_list,
            multisteps_list,
            current_expect_reward_array,
            discount_rate,
        )
